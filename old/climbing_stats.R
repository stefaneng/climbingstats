...
# Add year-month-day, month/year column and year column formated from the date
ws <- ws %>% mutate(year_month_day = format(date, "%Y-%m-%d"),
...
ggRoutesPlot <- ggplot(yds_routes, aes(x=reorder(simple_grade, -`times climbed`, sum), y=`times climbed`)) + 
  geom_bar(stat = "summary", fun.y=sum) + 
  xlab("Grade") + 
  ylab("Count") + 
  ggtitle("Rock Routes by Grade")
...
ggPitchesMonthYearPlot <- ggplot(ws, aes(x=reorder(month_year, date), y=`times climbed`)) +
  geom_bar(stat = "summary", fun.y=sum) +
  xlab("Month") +
  ylab("Pitches") +
  ggtitle("Pitches per Month")

# Viz 4: Pitches by Year
ggPitchesYearPlot <- ggplot(ws, aes(x=reorder(year, date), y=`times climbed`)) +
  geom_bar(stat = "summary", fun.y=sum) +
  xlab("Year") +
  ylab("Pitches") +
  ggtitle("Pitches per Year")

# Get first boulder problem at each grade
firstBoulderGradeDf <- boulders %>%
  group_by(simple_grade) %>%
  slice(which.min(date)) %>%
  select(name, grade = simple_grade, date = year_month_day, location)
